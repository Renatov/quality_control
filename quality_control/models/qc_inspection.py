...
class QcInspection(models.Model):
    ...
    def create(self, vals):
        if vals.get('name', '/') == '/':
            vals['name'] = 'QC-'+self.env['ir.sequence'].get('qc.inspection')
    ...
    def quality_test_check(self):
        if self.question_type == 'qualitative':
            ...
            if self.uom_id.id == self.test_uom_id.id:
                ...
                #nuevo_uom_id = self.env['qc.test.question'].search([('id','=', '1')])
                amount = self.uom_id._compute_quantity(self.quantitative_value, self.uom_id)
