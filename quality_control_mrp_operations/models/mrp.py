...
class MrpProductionWorkcenterLine(models.Model):
    ...
    required_test = fields.Boolean(related='operation_id.required_test', readonly=True)
    qtemplate_id = fields.Many2one(related='operation_id.qtemplate_id', string='Test')
    ...
    def create(self, data):
        ...
        idd = data.get('operation_id')
        test_required_id= workcenter_obj.search([('id','=',idd)]).required_test
        template_id = workcenter_obj.search([('id','=',idd)]).qtemplate_id
        data.update({'required_test': test_required_id, 'qtemplate_id': template_id})
        find_test = False
        if test_required_id:
            required_test = test_required_id
            if required_test:
                if 'qtemplate_id' not in data:
    ...
    def write(self, vals, update=False):
        workcenter_obj = self.env['mrp.routing.workcenter']
        if workcenter_obj.required_test in vals:
            required_test = workcenter_obj.required_test
            if required_test:
                if 'qtemplate_id' not in vals:
                    raise except_orm(_('Operation Modification Error!'),
                                     _("You must define the test template"))
